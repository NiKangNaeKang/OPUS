...
public class OrderServiceImpl implements OrderService{
	...
	public Map<String, Object> createOrder(OrderRequest request, int memberNo) {
        ...
        log.info("주문 생성 완료 - orderId: {}, memberNo: {}, amount: {}", 
                orderId, memberNo, request.getTotalAmount());
	...
	public TossPaymentResponse confirmPayment(PaymentConfirmRequest request, int memberNo) {
        ...
        try {
         ...
         // 6. 재고 차감 (결제 완료 시)
            ...
            log.info("결제 승인 완료 - orderId: {}, status: {}", 
...
log.info("가상계좌 입금 완료 처리 시작 - orderId: {}", orderId);
        ...
        if (!"가상계좌".equals(order.getPaymentMethod())) {
            log.error("가상계좌 주문이 아님 - orderId: {}, method: {}", 
        ...
        for (OrderItem item : orderItems) {
            ...
            if (item.getGoodsOptionNo() > 0) {
                ...
                if (stockResult == 0) {
                    log.error("재고 차감 실패 - orderId: {}, optionNo: {}, qty: {}", 
                             orderId, item.getGoodsOptionNo(), item.getQty());
                    throw new BusinessException("재고가 부족합니다.");
                }
                
                log.info("재고 차감 완료 - optionNo: {}, qty: {}", 
                        item.getGoodsOptionNo(), item.getQty());
            }
        }
    ...
    private void sendDepositConfirmEmail(Order order) {
        if (emailService != null) {
            emailService.sendDepositConfirmEmail(
                order.getEmail(),
